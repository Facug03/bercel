...
export default function AuthPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (mode === "register") {
      ...
      setMessage("Cuenta creada. Ya estás logueado.");
      setIsPending(false);
      return;
    }
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-lg items-center px-6 py-10 text-white">
      ...
      >
        ...
        <Card.Content className="p-0 pt-5">
          ...
          {session?.user ? (
            <div className="mt-5 rounded-xl border border-emerald-400/30 bg-emerald-400/10 p-3 text-sm text-emerald-200">
              Sesión activa como {session.user.email}
              <Button
                className="mt-3 w-full rounded-xl border border-white/20 text-white"
